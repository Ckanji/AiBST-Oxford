...
iris_raw %>% 
  group_by(species) %>% 
summarise(across(where(is.numeric),mean)) %>% 
  filter(sepal_length <6) %>% arrange(desc(sepal_length))
...
ggplot(exprs_assay_long, aes(x=value)) + 
  geom_histogram(fill = "black", bins = 50, aes(color=gene))+
  facet_wrap(~gene)
...
sample_info <- sample_info %>%
   mutate(T= as.factor(T), grade=as.factor(grade), N=as.factor(N), M=as.factor(M)) 
sample_info
```

...
```{r}
iris_features <- mutate(iris, Species=NULL) %>% as.matrix()
head(iris_features)
```
...
```{r}
max_absolute <- function(x)(which.max(abs(x)))
apply(pca_iris$rotation,2, max_absolute)
```


```{r}
pca_iris$rotation
max_index <- apply(pca_iris$rotation,2, max_absolute)
...
pca_iris_Petal.Length <- ggplot(pca_iris_dataframe,aes(x=PC1,PC2)) +
  geom_point(aes(color=Petal.Length))
pca_iris_Petal.Length
...
```{r}
variance_dataframe <- data.frame(explained_variance =explained_variance_ratio, pca=c("{PC1", "PC2", "PC3", "PC4")
...
```{r}
ggplot(variance_dataframe, aes(pca, explained_variance) ) +
  geom_col()

...
featurecounts_table <- read_tsv("allsamples_counts.tsv", comment = "#")
...
#This is how you can check if they are the same at once
...
rlog_transformed_dds <- rlog_transformed_dds<- rlog(dds_filtered, blind = TRUE)
...
```{r}
sizeFactors_df <- data.frame(
  y = sizeFactors(dds_filtered_updated)
)

ggplot(sizeFactors_df, aes(rownames(sizeFactors_df), y)) +
  geom_col()
...
results_obj_CD4_CD8 <- results(dds_final, alpha = 0.05, contrast = contrast)
...
shrunkenLFC_dds_results_obj <- lfcShrink(dds_final,coef = "cell_type_CD8_vs_CD4" )
...
shrunkenLFC_dds_results_df <- data.frame(shrunkenLFC_dds_results_obj)
head
# Use the org.Mm.eg.db package to get gene symbols for the ensembl gene ids (hint:you did this on bioconductor day) 
genesymbol <- mapIds(org.Mm.eg.db,
                     keys = rownames(shrunkenLFC_dds_results_df), column = "SYMBOL", 
                     keytype = "ENSEMBL")
genesymbol
...
shrunkenLFC_dds_results_df$Gene_symbol <- genesymbol
